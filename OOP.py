...
# 		# то есть при вызове функции super() мы получаем ссылку на базовый класс, то есть обращаемся к базовому классу. Из этого базового класса вызываем магический метод __new__ с аргументом cls - это ссылка класс как мы помним, и этот метод запускает процесс создания экземпляра класса и возвращает адрес нового созданного объекта, а в программе также его возращаем и он срабатывает
