...
# def __init__(self, x, y):...
# 								методы с параметром self обращаются к объекту класса и работают с переменными класса и предполагается что они вызываются из экземпляров класса и работает а атрибутами экмезмпляра через параметр self и также с атрибутами самого класса, доступ есть и к локальным атрибутам объекта и к атрибутам класса. Методы с параметром cls работает только с перменными класса можно к ним обращаться через параметр cls, и работать с атрибутами объектов класса не получится, так как нет ссылки self на объект. Методы без переменных self и cls это статические методы, они работают только внутри класса и не передают инфу в объект, вызывать их можно извне класса через обращение к классу, то есть "название класса.имя метода". Статическая функция не может создавать свойства объекта, но может например вывести на экран инфу из атрибутов объекта, то есть ее можно использовать как переменную класса через self.название статической функции
