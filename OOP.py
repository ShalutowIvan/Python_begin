...
class BookStudy:
	...
	def __hash__(self):
		return hash((self.name.lower(), self.author.lower(), self.year))
...
unique_books = set([hash(i) for i in lst_bs])
print(unique_books)
...
# Песнь 22. Наследование. Атрибуты private и protected!!!!!!!!!!!!!!!!

# режимы доступа:
# public - это когда перед атрибутом нет нижних подчеркиваний: attribute
# protected - это когда перед атрибутом есть одно нижнее подчеркивание: _attribute. Можно обращаться внутри класса и во всех дочерних классах
# private - это когда перед атрибутом есть 2 нижних подчеркивания: __attribute. Служит для обращения только внутри класса
# рассмотрим как режимы доступа влияют при наследовании классов
# у нас 2 класса с инициализаторами и делегированием. Сделаем все свойства объектов приватными

# class Geom:
# 	name = "Geom"
# 	def __init__(self, x1, y1, x2, y2):
# 		print(f"вызван инициализатор Geom для {self.__class__}")
# 		self.__x1 = x1
# 		self.__y1 = y1
# 		self.__x2 = x2
# 		self.__y2 = y2
...
# r = Rect(0, 0, 10, 20)
# print(r.__dict__)#тут у параметров в инициализаторе класса геом будут префиксы этого класса Geom, не смотря на то, что параметр self ссылается на объект класса Rect, и только у параметра fill будет префикс класса Rect, потому что он в инициализаторе класса Rect. {'_Geom__x1': 0, '_Geom__y1': 0, '_Geom__x2': 10, '_Geom__y2': 20, '_Rect__fill': 'red'} То есть все приватные свойства формируются таким образом. В том классе где они прописаны, тот префикс и добавляется. Из этого следует, что в дочернем классе мы не можем обратиться к этим свойствам. Можем в этом убедиться
# class Geom:
# 	name = "Geom"
# 	def __init__(self, x1, y1, x2, y2):
# 		print(f"вызван инициализатор Geom для {self.__class__}")
...
# 	def get_coords(self):
# 		return (self.__x1, self.y1)


# r = Rect(0, 0, 10, 20)
# r.get_coords()#будут ошибка AttributeError: 'Rect' object has no attribute '_Rect__x1'., то есть в это объекте не существует аттрибута _Rect__x1, это потому что у этого атрибута префикс класс Geom. НО если метод get_coords поместить в базовый класс, то ошибок не будет. Код ниже
# print(r.__dict__)
...
# r = Rect(0, 0, 10, 20)
# r.get_coords()#теперь ошибок не будет, но атрибуты будут класса геом, все кроме fill
# print(r.__dict__)
#такое поведение кажется странным, но вообще приватные атрибуты доступны только в самом классе, и не доступны в других классах, в том числе и в дочерних классах, это их стандартное свойство. То есть все атрибуты в инициализаторе можно использовать только в классе Geom, только внутри класса где они прописаны. Если нужно определить закрытые атрибуты во всех дочерних классах, то можно использовать определение protected, то есть одно подчеркивание
...
# r.get_coords()
# print(r.__dict__)
# # протектед не запрещает обращаться к атрибуту извне
# print(r._x1)#можно обращаться так. protected предупреждает программиста, что так лучше не обращаться, но на выполнение программы не влияет. но в дальнейшем может привести к проблемам, когда этих атрибутов уже не будет
# Также себя ведут и атрибуты класса, например name в классе Geom

# class Geom:
# 	__name = "Geom"#теперь это свойство не доступно извне
# 	def __init__(self, x1, y1, x2, y2):
# 		print(f"вызван инициализатор Geom для {self.__class__}")
# 		self._x1 = x1
# 		self._y1 = y1
# 		self._x2 = x2
# 		self._y2 = y2
# 		self._name = self.__name#так можно обратиться так как __name находится в текущем классе, и свойство объекта будет принадлежать объекту self, то есть Rect например
...
# 		super().__init__(x1, y1, x2, y2)
# 		self._fill = fill
# 		# self.__name = Geom.__name#тут будет ошибка, так нельзя обратиться
...
# print(r.__dict__)
# протектед не запрещает обращаться к атрибуту извне
# print(r.name)#в классе rect и его объекте это свойство не доступно
# Также можно и методы приватными делать
...
# 		self._y2 = y2
# 		self._name = self.__name#так можно обратиться так как __name находится в текущем классе, и свойство объекта будет принадлежать объекту self, то есть Rect например
# 		self.__verify_coord(x1)#тут ошибки не будет
...
# 	def __init__(self, x1, y1, x2, y2, fill="red"):
# 		super().__init__(x1, y1, x2, y2)
# 		self.__verify_coord(x1)#тут будет ошибка. Но если прописать protected _, то ошибка в дочерних классах не будет
# 		self._fill = fill
# 		# self.__name = Geom.__name#тут будет ошибка, так нельзя обратиться
...
# # print(r1.get_rect_pr(), r2.get_rect_pr())#вывели периметры на экран
# # print(s1.get_sq_pr(), s2.get_sq_pr())
# #представим что нам нужны периметры для соответствующих объектов и у нас есть список объектов
# geom = [r1, r2, s1, s2]
# # for g in geom:
# # 	print(g.get_rect_pr())#если так прописать, то когда дойдем до объекта квадратов s1 или s2 будет ошибка (AttributeError: 'Square' object has no attribute 'get_rect_pr'), так как в классе Square нет метода get_rect_pr

# # самый просто вариант это прописать условие для проверки типов объектов и вызывать нужный нам метод
# for g in geom:
...
#но такая реализация не очень хорошая реализация. В случае если у нас будут другие классы с другими объектами, то будет больше условий и наш код будет не универсален
#добавим еще один класс треугольник тоже с вычислением периметра
...
# geom = [r1, r2, s1, s2, t1, t2]#добавили объекты в список
...
# 		print(g.get_rect_pr())
# 	else:
# 		print(g.get_sq_pr())#тут будет ошибка AttributeError: 'Triangle' object has no attribute 'get_sq_pr'. Так как в треугольниках нет метода get_sq_pr

# в этом случае можно применить технлогию полиморфизм. Как это сделать. Описание ниже
# Определим метод с одним и тем названием во всех трех классах
...
# список объектов можно создать и по другому
# geom = [Rectangle(1, 2), Rectangle(3, 4), Square(10), Square(20), Triangle(1, 2, 3), Triangle(4, 5, 6)]#это будет тоже самое что и записи выше

# for g in geom:
# 	print(g.get_pr())#теперь все будет работать без ошибок. Причем метод get_pr называется везде во всех классах одинаково и для каждого объекта будет вызван свой метод get_pr со своей реализацией из нужного класса

# но есть один недостаток. Если в каком либо классе нет такого метода get_pr, то опять будет ошибка. Первый способ это создать базовый класс для всех классов. И все классы сделать наследниками. Метод get_pr оставить, и в случае если гдето его не будет, то сработает метод базового класса 
...
# for g in geom:
# 	print(g.get_pr())
#для треугольника сработает метод базового класса, так как мы закоментировали метод get_pr в нем
# Но в идеале лучше сделать, чтобы все же реализацию метода get_pr была даже если он там не прописан. Как это сделать. Для начала можно сделать вызов исключения в базовом классе
# class Geom:
# 	def get_pr(self):
# 		raise NotImplementedError("в дочернем классе должен быть переопределен метод get_pr()")#мы будем видеть с чем связано исключение. В нашем случае оно будет вызвано для класса треугольников. Если его определить, то ошибка уйдет
...
# В языках программирования методы которые не имеют своей собственной реализации и которые нужно обязательно переопределять в дочерних классах называются абстрактными. В питоне нет чисто абстрактных методдов, здесь мы выполняем имитацию их поведения заставляя программиста переопределять метод в дочерних классах, самостоятельно геренируя исключение NotImplementedError, чтобы потом знать что нужно переопределить метод в нужном классе. в других ЯП есть абстрактные классы


# Песнь 24. Множественное наследование!!!!!!!!!!!!!!!!!!!!!!
# Множественное наследование - это когда один дочерний класс наследуется от нескольких базовых. Такой механизм допускается в питоне
# выглядит это так:
# class A(B1, B2, B3, ..., BN):
# Один из примеров это миксины(mixins) - примеси
# Представим что мы делаем инет магазин по продаже товаров. Каждый товар будет определяться своим классом. Товары: ноутбуки, ЖД, процессоры... Классы: class Notebook, class SSD, class CPU. 
# У всех классов будет один базовый класс Goods
# У каждого товара будут след поля Параметры:  name - наименование, wieght - вес, price - цена
# class Goods:
# 	def __init__(self, name, weight, price):#тут просто инициализатор
# 		print("init MixinLog")
# 		self.name = name
# 		self.weight = weight
# 		self.price = price
...
# 	def print_info(self):
# 		print(f"{self.name}, {self.weight}, {self.price}")#метод для вывода инфы о товаре

# # сделаем дочерний класс

# class Notebook(Goods):
# 	pass


# n = Notebook("Acer", 1.5, 30000)#тут сработает инициализатор базового класса
# n.print_info()
# представим что нам нужно сделать логирование товаров магазина. Это можно сделать в самом базовом классе, или в дочернем классе, но это плохая идея. Правильно сделать еще один отдельный класс для логирования, он не будет наследоваться ни от какого класса, будет самостоятельным. В нем будет логирование
...
# 		#тут мы сделали отдельный независимый класс с функционалом логирования, использует id для товаров. Такие классы которые добавляют дополнилеьный функционал и называют миксинами - примеси


# # сделаем дочерний класс и добавим в качестве родителей 2 класса, получается множественное наследование

# class Notebook(Goods, MixinLog):#теперь класс Notebook наследуется от двух классов и в несть функционал этих двух классов
# 	pass
...
# n.print_info()
# n.save_sell_log()
# при запуске программы сейчас будет ошибка AttributeError: 'Notebook' object has no attribute 'id'.
# Это потому что не был выполнен инициализатор класса MixinLog. Как это произошло. При создании объектов инициализатор ищется в дочернем классе, а затем в базовых классах. Сначала в первом базвом классе Goods из тех классов которые мы перечислили в скобках дочернего класса, если не будет найден, то потом во втором MixinLog. Если он (инициализатор) найдется в первом, то во втором уже поиск не будет идти и во втором классе инициализатор уже не будет вызван. Но нужно чтобы инициализатор из второго класса тоже был вызван. Как это сделать. Код ниже

# class Goods:
# 	def __init__(self, name, weight, price):#тут просто инициализатор
# 		super().__init__(1)#тут передали параметр, он тут просто так для примера
# 		#это можно прописать так. И тут вызовется инициализатор класса MixinLog. НО вопрос откуда функция super знает что нужно вызвать инициализатор класса MixinLog, а например не к базовому классу object. В питоне есть специальнйы алгоритм обхода базовых классов - MRO - Method Resolution Order при множественном наследовании. Обход будет идти так. Сначала идет дочерний класс, потом первый родитель, затем второй родитель и тд, и в конце после всех родителей базовый класс object. В нашем случае будет так: Notebook, Goods, MixinLog, object. Также есть магический метод для проверки этой последовательности __mro__, она выведет последовательность этой цепочки для любого класса __mro__
# 		print("init Goods")
# 		self.name = name
# 		self.weight = weight
# 		self.price = price
...
# 	def __init__(self, p1):#указали параметр
# 		super().__init__()
# 		print("init MixinLog")
# 		MixinLog.ID += 1
# 		self.id = MixinLog.ID
...
# 	def __init__(self):#тут сделали еще одного родителя
...
# class Notebook(Goods, MixinLog, MixinLog2):#теперь класс Notebook наследуется от двух классов и в несть функционал этих классов. Менять их местаии нельзя, так как нарушится логика работы с базовыми классами и питон выдаст ошибку. Мы выше прописали порядок инициализаторов. Ниже описание как это работает. 
# 	pass

# class Notebook(MixinLog, Goods):#так питон напишет ошибку, потому что порядок нарушили написания базовых классов
# 	pass

# n = Notebook("Acer", 1.5, 30000)#тут сработает инициализатор базового класса
# n.print_info()
# n.save_sell_log()
# # print(Notebook.__mro__)#(<class '__main__.Notebook'>, <class '__main__.Goods'>, <class '__main__.MixinLog'>, <class 'object'>) это выведется, кортеж из названий классов. Для других классов будет другая последовательность, но первый базовый класс после дочернего он всегда первый из списка, это важный момент, потому что мы всегда можем быть уверены, что инициализатор первого базового класса сработает первым и это можно использовать
# Когда мы создаем объект, то передаем в него аргументы, и мы уверены что эти аргументы будут переданы именно в инициализатор первого базового класса, а не в какой то другой инициализатор другого базового класса, и на это расчитываем, будет гарантия что все сработает при множественном наследовании. ПОлучается порядок написания базовых классов в классе наследнике имеет очень важное значение. Если его прописать не верно то будет ошибка. Обычно в других базовых класса не прописывают аргументы в инициализаторе, так как другие базовые классы это дополнительный функционал. В последующих базовых классах лучше прописывать один параметр self в инициализаторе, и никаких дополнительных. Потому что потом могут быть сложности, так как их тоже нужно прописывать при делегировании, то есть указать при вызове инициализатора в первом ините. Можно прописать и далее еще другие классы, и тогда там нужно поочередно писать в каждом из них инит для следующего класса. То есть в первом базовом классе инит для второго родителя, во втором родителе инит для третьего родителя и тд, тогда все иниты отработают, но все параметры в каждом ините нужно учитывать или не прописывать вообще, так как если нарушить порядок прописания базовых классов, то питон будет писать ошибку. Все нужные парметры прописать в первом базовом классе, в других только доп функционал и все, лучше стараться делать так для избежания ошибок.
# print(Notebook.__mro__)

# Вызов методов с одинаковыми именами в базовых классах!!!!
# Предположим что в классах Goods и MixinLog есть одинаковые методы
...
# 	def print_info(self):
# 		MixinLog.print_info(self)#переопредили метод в дочернем классе, он будет напрямую вызываться из класса MixinLog. Теперь его можно просто вызвать через объект

# n = Notebook("Acer", 1.5, 30000)#тут сработает инициализатор базового класса
# # n.print_info()#этот метод будет вызван из класса Goods. Так как он первым нашелся и далее не дерегировался 
# # как сделать так чтобы переопределнный метод print_info вызывался из класса MixinLog
# # самое простое это просто вызвать его через класс
# # MixinLog.print_info(n)#передать параметр нужно обязательно, потому что там есть параметр self, это параметр для передачи объекта, то есть ссылка на объект
# # Но лучше прописать его в дочернем методе, переопределив его там с таким же названием, и в самом методе его вызвать
# n.print_info()#теперь он будет из MixinLog. Это нужно только в том случае если такой метод есть в нескольких базовых классах, если только в одном, то переопределять не нужно


# Песнь 25.1. Коллекция __slots__!!!!!!!!!!!!!!!!!!!!!
#__slots__ это специальная коллекция ей можно пользоваться в питоне
# class Point:
#     def __init__(self, x, y):
#         self.x = x
#         self.y = y
#пока нет коллекции __slots__ можно свободно опероровать существующими свойствами. То есть изменять их и добавлять нвоые свойства в объекты этого класса
# pt = Point(1, 2)
# pt.x = 5
# pt.y = 11
# pt.z = 55
# print(pt.__dict__)#тут свойства будут изменеными, и будет новое свойство z
#если нам нужно объявить класс для отображения точек на плоскости у которого будут только те свойства которые мы прописали при создании объекта, то есть только x и y, то можно применить коллекцию __slots__. Обычно она определяется через кортеж
#сделаем еще один класс
# class Point2D:
#     __slots__ = ("x", "y")#тут мы пишем кортеж из строк - названий локальных свойств объектов класса, которые будут разрешены в объектах класса. При создании объектов другие названия свойств объекта нельзя писать. В самом классе можно писать другие переменные и проводить с ними вычисления
#     # MAX_COORD = 100#прописали свойство класса, питон ошибок не выдаст
#     def __init__(self, x, y):
#         self.x = x
#         self.y = y
#
# pt2 = Point2D(10, 20)
# pt2.x = 11
# pt2.y = 22#тут пока ошибок не будет
# pt2.z = 1#тут питон выдаст ошибку (AttributeError: 'Point2D' object has no attribute 'z'), потому что в классе запрещены другие свойства
# print(pt2.__dict__)#тут питон тоже выдаст ошибку (AttributeError: 'Point2D' object has no attribute '__dict__'.), так как если мы прописали коллекцию __slots__, то коллекция __dict__, в нем не формируется. Поэтому мы и не можем добавлять локальные другие свойства. То есть получается коллекция __slots__ подменяет коллекцию __dict__. Существующие локальные свойтства можно менять как нам угодно. Можно их и удалять, и добавлять потом заново, но только имена можно добавлять только те, которые есть в коллекции __slots__.
# print(pt2.MAX_COORD)#вывели свойство класса, ошибок нет, все норм. так как это не свойтсва объекта, а свойство класса.
#Также уменьшается объем памяти занимаемые объектами класса. Это можно проверить
# print(pt.__sizeof__() + pt.__dict__.__sizeof__())#тут будет 304
# print(pt2.__sizeof__())#тут будет 32. Так как тут нет прострнатсва имен __dict__
#также коллекция __slots__ ускоряет работу с переменными объекта x и y. Пропишем метод для небольшого вычисления в обоих классах
#
# import timeit#импортировали модуль для подсчета времени
#
# class Point:
#     def __init__(self, x, y):
#         self.x = x
...
# p = Point(1, 2)
# p2 = Point2D(10, 20)
# t1 = timeit.timeit(p.calc)#передали в функцию модуля для подсчета времени нашу функцию, у которой мы хотим посмотреть время работы
# t2 = timeit.timeit(p2.calc)
# print(t1, t2)#в уроке препод говорит, что с функцией __slots__ работает быстрее, но у меня наоборот, с функцией __slots__ немного медленее получилось, второй объект работает немного медленее. Возможно влияет версия питона
#приемущества __slots__:
# - ограничение создаваемых локальных свойств объекта
# - уменьшение занимаемой памяти
# - ускорение работы с локальными свойствами (спорный момент)

#для коллекции слотс можно юзать сеттеры и геттеры, ошибок не будет
#при наследовании коллекция слотс не наследуется, создается новое пространство имен с коллекцией дикт. В нее будут входить только новые свойства, а свойства из коллекции слотс не входят
#если в наслденике прописать коллекцию слотс, даже пустую, то она просто создаться, и удалит коллекцию __dict__, и сама коллекция слост унаследуется от родителя. Если коллекцию слотс в наследнике прописать с каким либо значение(значением обязательно должен коллекция - например кортеж), то коллекции слотс из наследника и родителя суммируются
