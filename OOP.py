...
# то есть 2 этих класса, это 2 пространства имен они пока не связаны между собой. Но можно настроить связь, чтобы открытые атрибуты класса Geom были доступны в классе Line
...
#если чертить блок схему, то отображать их нужно в виде прямоугольников, и стрелку чертим от дочернего класса к базовому
...
# Тут мы в классе Geom мы прописали общий метод
class Geom:
	name = "Geom"
	def set_coords(self, x1, x2, y1, y2):
		self.x1 = x1
...
r = Rect()
l.set_coords(1, 1, 2, 2)#теперь через объекты можно обращаться к методу из базового класса
