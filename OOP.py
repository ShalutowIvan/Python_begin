...
# в питоне есть функция hash позволяющая вычислять hash для неизменямых объектов. Для ранвых объектов будет получаться вна выходе одинаковый hash.
# print(hash(123))
# print(hash("Python"))
# print(hash("Python"))
# print((1, 2, 3))
...
# ХЕШ МОЖНО ВЫЧИСЛЯТЬ ТОЛЬКО ДЛЯ НЕИЗМЕНЯЕМЫХ ОБЪЕКТОВ!
# print(hash([1, 2, 3]))#будет TypeError: unhashable type: 'list'
# зачем все это нужно. Некоторые объекты в питоне используют объекты в качестве своих ключей. Например словари
# в качестве ключа у словаря должен быть неизменяемый объект. НАпример список в качестве ключа нельзя использовать. 
# a = {}
# a[[1, 2]] = {1}#тут будет ошибка TypeError: unhashable type: 'list'. ТО есть тоже ошибка как и выше с хешем
# d = {}
# d[5] = 5
# d["python"] = "python"
# d[(1, 2, 3)] = [1, 2, 3]
# ([хэш ключа, ключ])
# по факту ключ хранится так: хэш ключа, ключ. То есть когда мы создаем элемент словаря, то создается кортеж: хэш ключа, и сам ключ
# Для чего это нужно. первоначально нужная запись в словаре ищется по хешу. Потому что существует быстрый алгоритм поиска нужного значения хэша, а затем для равных хэшей отбирается запись с указанным в ключе объекте
...
p1 = Point(1, 2)
p2 = Point(1, 2)
print(hash(p1), hash(p2), sep="\n")#объекты которые мы сами создаем они воспринимаются питоном как неизменяемые и к ним можно применить функцию хэш и вычислить его. Причем в нашем случае хэши будут разные, не смотря на что координаты будут точек одинаковые. То есть хэш видит, что это объекты разные
print(p1 == p2)#тут будет выведен фолз, это означает что у них хэши разные и они не равны друг другу
