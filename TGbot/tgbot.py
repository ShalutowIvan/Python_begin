...
# фишки ботов с хендлерами. lambda фильтр и фильтр Text. код в файле ficha.py
# с обычными кнопками нельзя сделать так, чтобы в них был написан нужный нам текст, там всегда будет именно команда. Чтобы был написан нужнйы нам текст, нужно сделать инлайн кнопки, они отправляются прямо в чат. В них можно записывать что угодно, любые действия или ссылки на сайты. Напишем парсинг сообщей в файле ficha

# урок № 7 База данных для бота sqlite
#сделаем базу данных для админки, то есть для раздела admin

#урок № 8 Инлайн inline кнопки
#будет делать отдельного бота для теории. Инлайн кнопки будут использоватьяс потом для корректировки БД пиццерии. Код по теории инлайн кнопок в файле inline.py
